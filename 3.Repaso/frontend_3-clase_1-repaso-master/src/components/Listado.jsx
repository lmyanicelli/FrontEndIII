...
const Listado = ({ listadoDeFrases }) => {
    ...
    //refactorizo con slice(-5) obtengo los últimos 5 elementos del array,
    //reverse invierte el orden para q aparezca primero la última frase
    //map crea el array con los elementos p
    return (
        <div className="app__viejasFrases">
            <h3>ULTIMAS 5 FRASES</h3>
            {frasesState.slice(-5).reverse().map(function (element, index) {
        return <p>{element}</p>
     })}
